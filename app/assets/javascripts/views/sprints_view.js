...
App.Views.Sprints = {
  Show: App.Views.BaseView.extend({
    ...
    updateStatistics: function(attributes) {
      ...
      $('#backlog-data-area .backlog-stats').html( JST['templates/sprints/stats']({ attributes: attributes }) );

      var totals = this.$('.stories-container .totals');
      if (this.model.SprintStories().length === 0) {
        totals.addClass('notice').html( JST['templates/sprints/empty']() );
      } else {
        totals.removeClass('notice').html( JST['templates/sprints/totals']({ attributes: attributes, storyCount: this.model.SprintStories().length, sprint: this.model }) );
      }
    ...
    updateSprintButtonsView: function() {
      var that = this;
      var completeView = $('<div>' + JST['templates/sprints/show']({ model: that.model }) + '</div>');
    ...
    bulkMoveStories: function(event) {
      ...
      $('body').append(JST['templates/sprints/move-dialog']({ sprints: eligibleSprints }));
