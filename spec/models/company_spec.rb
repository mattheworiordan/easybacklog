...
describe Company do
  ...
  it 'should only allow a rate if velocity is present' do
    expect { Factory.create(:company, :default_rate => 50, :default_velocity => nil)}.to raise_error ActiveRecord::RecordInvalid, /Default rate cannot be specified if default velocity is empty/
