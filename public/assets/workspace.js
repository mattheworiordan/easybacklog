...
var slice=ArrayProto.slice,concat=ArrayProto.concat,unshift=ArrayProto.unshift,toString=ObjProto.toString,hasOwnProperty=ObjProto.hasOwnProperty;
...
i<l;
i++){if(i in array&&array[i]===item){return i
}}return -1
};
...
if(!_.isArguments(arguments)){_.isArguments=function(obj){return !!(obj&&hasOwnProperty.call(obj,"callee"))
}
...
list.tail=tail.next={};
return this
},unbind:function(ev,callback){var calls,node,prev;
if(!ev){this._callbacks=null
}else{if(calls=this._callbacks){if(!callback){calls[ev]={}
}else{if(node=calls[ev]){while((prev=node)&&(node=node.next)){if(node.callback!==callback){continue
}prev.next=node.next;
node.context=node.callback=null;
break
}}}}}return this
},trigger:function(eventName){var node,calls,callback,args,ev,events=["all",eventName];
if(!(calls=this._callbacks)){return this
}while(ev=events.pop()){if(!(node=calls[ev])){continue
...
_.extend(Backbone.Model.prototype,Backbone.Events,{_changed:false,idAttribute:"id",initialize:function(){},toJSON:function(){return _.clone(this.attributes)
...
},set:function(key,value,options){var attrs;
...
if(!_.isEqual(now[attr],val)||(options.unset&&(attr in now))){options.unset?delete now[attr]:now[attr]=val;
delete escaped[attr];
this._changed=true;
if(!options.silent){this.trigger("change:"+attr,this,val,options)
}}}if(!alreadyChanging){if(!options.silent&&this._changed){this.change(options)
}this._changing=false
}return this
},unset:function(attr,options){(options||(options={})).unset=true;
return this.set(attr,null,options)
},clear:function(options){(options||(options={})).unset=true;
...
},url:function(){var base=getValue(this.collection,"url")||getValue(this,"urlRoot")||urlError();
...
},changedAttributes:function(now){if(!this._changed){return false
}now||(now=this.attributes);
var changed=false,old=this._previousAttributes;
for(var attr in now){if(_.isEqual(old[attr],now[attr])){continue
...
},_performValidation:function(attrs,options){var error=this.validate(attrs,options);
...
i<l;
i++){if(options&&(options.at==+options.at)&&i){options.at+=1
...
return matched
},navigate:function(fragment,options){if(!options||options===true){options={trigger:options}
}var frag=(fragment||"").replace(hashStrip,"");
if(this.fragment==frag||this.fragment==decodeURIComponent(frag)){return
}if(this._hasPushState){if(frag.indexOf(this.options.root)!=0){frag=this.options.root+frag
}this.fragment=frag;
window.history[options.replace?"replaceState":"pushState"]({},document.title,frag)
}else{this.fragment=frag;
this._updateHash(window.location,frag,options.replace);
if(this.iframe&&(frag!=this.getFragment(this.iframe.location.hash))){if(!options.replace){this.iframe.document.open().close()
}this._updateHash(this.iframe.location,frag,options.replace)
}}if(options.trigger){this.loadUrl(fragment)
}},_updateHash:function(location,fragment,replace){if(replace){location.replace(location.toString().replace(/(javascript:|#).*$/,"")+"#"+fragment)
...
_.extend(Backbone.View.prototype,Backbone.Events,{tagName:"div",$:function(selector){return(selector==null)?$(this.el):$(selector,this.el)
...
},delegateEvents:function(events){if(!(events||(events=getValue(this,"events")))){return
...
}if(!options.data&&model&&(method=="create"||method=="update")){params.contentType="application/json";
...
var getValue=function(object,prop){if(!(object&&object[prop])){return null
}return _.isFunction(object[prop])?object[prop]():object[prop]
};
var urlError=function(){throw new Error('A "url" property or function must be specified')
};
var wrapError=function(onError,originalModel,options){return function(model,resp){var resp=model===originalModel?resp:model;
if(onError){onError(model,resp,options)
}else{originalModel.trigger("error",model,resp,options)
}}
}
}).call(this);
/*!
...
$(target).append(form);
setTimeout(function(){targetProps.inputHeight=$(elem).height()
},1);
$(elem).focus();
$(elem).blur(function(event){if(!eventAlreadyProcessed){eventAlreadyProcessed=true;
...
$(elem).click(function(event){event.stopPropagation()
});
...
elem.autocomplete({source:($.isArray(options.autoComplete)?options.autoComplete:options.autoComplete()),minLength:0,delay:0})
}}return elem
};
...
$(window).resize(function(e){var resizeWidthBy=$(window).width()-windowWidth,moveTabsBackBy;
...
if(moveTabsBackBy){scrollableTabOffset+=moveTabsBackBy;
...
slideTabsAfterResize=function(scrollableTabOffset,scrollerWidth,scroller){if(scrollableTabOffset<0){if(scrollableTabOffset+totalElemWidth(scroller.find("li"))+50<scrollerWidth){return -(scrollableTabOffset+totalElemWidth(scroller.find("li"))+50-scrollerWidth)
}}return false
};
...
}function unUrlify(target){$(target).find("a.urlified").each(function(index,node){var href=$("<div>").html($(node).attr("href")).text();
$(node).replaceWith($("<span>").text(href))
})
}function niceNum(value){if(value){return String(value).replace(/\.0+$/,"")
}else{return value
...
}var I18n=(function(){return{t:function(value,defaultValue){if(this[value]){return this[value]
}else{return defaultValue
}},add:function(localizations){_(this).extend(localizations)
}}
})();
...
$("#dialog-confirm").dialog({resizable:false,height:160,modal:true,buttons:buttons});
return(false)
});
var topDropdowns=$("#user-account-dropdown, #account-dropdown");
topDropdowns.on("click",function(e){var dropdown=this,alreadyActive=$(dropdown).hasClass("active");
if($(this).is("#account-dropdown")){$("#account-dropdown .top-nav-dropdown-overlay").css("left",($("#account-dropdown").offset().left-30)+"px").css("right","10px").width("auto")
}topDropdowns.removeClass("active");
if(alreadyActive){$(dropdown).removeClass("active")
}else{$(dropdown).addClass("active")
}e.preventDefault();
e.stopPropagation();
$(document).on("click.user-account",function(e){$(dropdown).removeClass("active");
$(this).off("click.user-account")
})
}).on("click",".top-nav-dropdown-overlay a",function(e){topDropdowns.off("click");
...
window.JST["_shared/backlog_stats"]=_.template("<% var pointsText = addCommas((model.get('points') || 0).toFixed(1)) + ' ' + (model.get('points') == 1 ? I18n.t ('point') : I18n.t ('points')) %>\n<% if (!useDaysEstimates) { %>\n  <strong><%=pointsText%></strong>\n<% } else { %>\n  <%=pointsText%>\n<% } %>\n<% if (useCostEstimates) { %>\n  / <%= model.get('cost_formatted')%>\n<% } %>\n<% if (useDaysEstimates) { %>\n  / <strong><%= addCommas((model.get('days') || 0).toFixed(1)) %> <%= model.get('days') == 1 ? I18n.t ('day') : I18n.t ('days') %></strong>\n<% } %>");
...
window.JST["acceptance_criteria/show"]=_.template('<div class="index<% if (model.IsEditable()) { %> cross-hair-enabled<% } %>" title="Drag to reorder">&nbsp;</div>\n<div class="cross-hair-indicator<% if (model.IsEditable()) { %> cross-hair-enabled<% } %>">&nbsp;</div>\n<div class="data"><%= urlify(multiLineHtmlEncode(model.get(\'criterion\')), 35) %></div>');
...
window.JST["backlogs/stats"]=_.template("<div class=\"title\"><%= I18n.t ('backlog_total') %>:</div>\n<div class=\"output\">\n  <%= JST['_shared/backlog_stats']({ model: model, useDaysEstimates: useDaysEstimates, useCostEstimates: useCostEstimates }) %>\n</div>");
...
window.JST["stories/status-drop-down"]=_.template("<div id=\"sprint-story-status-dropdown\">\n  <ul>\n    <% App.Collections.SprintStoryStatuses.each(function(status) { %>\n      <% if (status.get('code') !== model.SprintStory().Status().get('code')) { %>\n        <li id=\"status-id-<%= status.get('id')%>\" class=\"status-code-<%= status.get('code')%>\"><%= status.get('status')%></li>\n      <% } %>\n    <% }); %>\n  </ul>\n</div>");
